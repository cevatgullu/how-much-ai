  ...
  async () => {
    ...
    const { stdout, stderr } = await runPowerShellFile(
      [
        ...
        "$script:taskStore = @{}; $script:registrationCount = 0; $script:unregistrationCount = 0; $script:listenerMode = 'none'; $script:listenerTerminations = 0; $script:listenerAddressPrefilters = 0; $script:listenerPortPrefilters = 0; $script:serviceCommand = ''; $script:updateFault = ''; $script:edgeLive = $false",
        ...
        "function Register-ScheduledTask { [CmdletBinding()] param([string]$TaskName,$Action,$Trigger,$Principal,$Settings,[switch]$Force) $task = [pscustomobject]@{ TaskName = $TaskName; Actions = @($Action); Triggers = @($Trigger); Principal = $Principal; Settings = $Settings; State = 'Ready' }; $script:taskStore[$TaskName] = $task; $script:registrationCount += 1; if ($script:updateFault -ceq ('task-' + $TaskName)) { $script:updateFault = ''; throw 'Injected task replacement failure.' }; return $task }",
        ...
        "function Unregister-ScheduledTask { [CmdletBinding()] param([string]$TaskName,[switch]$Confirm) $script:unregistrationCount += 1; $script:taskStore.Remove($TaskName) | Out-Null }",
        "function Stop-ScheduledTask { [CmdletBinding()] param([string]$TaskName) if ($script:taskStore.ContainsKey($TaskName)) { $script:taskStore[$TaskName].State = 'Ready' } }",
        ...
        "$script:updateFault = ''; $script:failRollback = $false; function Move-Item { [CmdletBinding()] param([string]$LiteralPath,[string]$Destination,[switch]$Force) $moveArgs = @{ LiteralPath = $LiteralPath; Destination = $Destination }; if ($Force) { $moveArgs.Force = $true }; Microsoft.PowerShell.Management\\Move-Item @moveArgs -ErrorAction Stop; $destinationFull = [IO.Path]::GetFullPath($Destination); if ($script:failRollback -and $destinationFull -ceq [IO.Path]::GetFullPath((Join-Path (Join-Path $state 'runtime') $releaseACommit))) { $script:failRollback = $false; throw 'Injected rollback interruption.' }; $matchesFault = (($script:updateFault -ceq 'runtime' -and $destinationFull -ceq [IO.Path]::GetFullPath((Join-Path (Join-Path $state 'runtime') $releaseBCommit))) -or ($script:updateFault -ceq 'bootstrap' -and $destinationFull -ceq [IO.Path]::GetFullPath((Join-Path $state 'bootstrap'))) -or ($script:updateFault -ceq 'control' -and $destinationFull -ceq [IO.Path]::GetFullPath((Join-Path $state 'install.json'))) -or ($script:updateFault -ceq 'shortcut' -and $destinationFull -ceq [IO.Path]::GetFullPath($mainLauncher))); if ($matchesFault) { $script:updateFault = ''; throw 'Injected promotion failure.' }; if ($script:updateFault -ceq 'final' -and $destinationFull -ceq [IO.Path]::GetFullPath($mainLauncher)) { $script:updateFault = ''; [IO.File]::WriteAllText((Join-Path (Join-Path (Join-Path $state 'runtime') $releaseBCommit) 'package.json'),'final-verification-fault') } }",
